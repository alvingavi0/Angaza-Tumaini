#!/usr/bin/env python3
"""Fix team member cards using latin-1 encoding"""

# Read with latin-1 encoding (handles all bytes)
with open('index.html', 'r', encoding='latin-1') as f:
    content = f.read()

print(f"File size: {len(content)} chars")

# Count occurrences of the old team card pattern
old_class_count = content.count('team-member bg-white p-6')
flip_class_count = content.count('team-member-front')

print(f"Old team-member cards: {old_class_count}")
print(f"New flip cards: {flip_class_count}")

# Replace the Evans card (still has old structure)
# Looking for the second article in the 2-col grid
evans_old = '''<article class="team-member bg-white p-6 rounded-xl shadow-md scroll-reveal">
...
if evans_old in content:
    ...
    print("⚠️ Evans card pattern not found")

# Now fix the Erick/Krista card (it's a hybrid right now)
# Find the mixed content and replace it
erick_old = '''<div class="text-sm text-gray-500 mb-3">Founders & Missionaries</div>
...
erick_new = '''<p class="text-sm text-gray-600">Founders & Missionaries</p>
                                    <p class="text-xs text-gray-500 mt-4">Hover to flip →</p>
                                </div>
                                <div class="team-member-back rounded-xl">
...
if erick_old in content:
    ...
    print("✅ Fixed Erick/Krista card (hybrid)")
else:
    print("⚠️ Erick/Krista hybrid pattern not found")
